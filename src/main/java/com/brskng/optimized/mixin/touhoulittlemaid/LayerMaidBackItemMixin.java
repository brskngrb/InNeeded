...
@Pseudo @Mixin(targets="com.github.tartaricacid.touhoulittlemaid.client.renderer.entity.layer.LayerMaidBackItem", remap=false)
public abstract class LayerMaidBackItemMixin {
    ...
    private void skipLod(PoseStack ps, MultiBufferSource bs, int l, Mob maid, float a, float b, float c, float d, float e, float f, CallbackInfo ci){
        if(BrskngsOptimizedClientConfig.TLM_LOD_DISABLE_BACK_ITEM_LAYER.get() && TouhouLittleMaidLodHelper.shouldUseLod(maid)) ci.cancel();
    }
}
